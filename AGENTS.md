...
- **Backups**: `services/backup-databases.sh` (root crontab, 04:30 daily → `/var/log/bdsk-backup.log`) dumps the `chatwoot_dev` Postgres database and takes consistent SQLite snapshots of all three agent databases into `/root/backups/behdashtik`, keeping 14 days. Nothing else on this box is backed up — the `.env` files are still single-copy.
- **Basalam is a second agent**, and it lives in this repo: `services/basalam-agent` (systemd `bdsk-basalam-agent`, port 8104, Chatwoot inbox 13) plus `services/basalam-bridge` (`bdsk-basalam-bridge`, port 8103), which mirrors Basalam chat into that inbox. Both are forks of the website agent and drift from it — fix a bug in both trees. Their `.env`, `data/` and `.venv/` are gitignored, so a fresh clone needs those recreated; the running copies are the ones under `services/`, referenced by absolute path from the unit files and from `BDSK_BSLM_DB_PATH` in the bridge `.env`.
